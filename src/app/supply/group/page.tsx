...
import { type Group, columns } from "./columns";
        ...
        <main className="flex flex-col h-[calc(100vh-1rem)] w-full">
            ...
            <section className="flex flex-col items-center h-screen w-full">
                <div className="flex items-center justify-between w-full px-4 py-2  shadow-md">
                    ...
                    <Modal modal={{ labelButton: "Novo Grupo", title: "Adicionar Grupos" }}>
                        <GroupForm valueToken={token?.value || ''} />
                    </Modal>
                </div>
                <div>
                    <DataTable columns={columns} data={datas.groups} />
                </div>
            </section>
        </main>
    )
